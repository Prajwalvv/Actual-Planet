"""
EXPORT SYNTHESIZER MODEL TO ONNX

Converts the trained PyTorch model to ONNX format for production inference.

Usage:
    python export_synthesizer_onnx.py --model-path ../../models/agents/synthesizer/synthesizer_model.pt
"""
...
from synthesizer_model import SynthesizerPointwiseModel
...
def export_to_onnx(
    ...
    # Load trained model
    model = SynthesizerPointwiseModel(input_dim=input_dim, hidden_dim=hidden_dim)
    ...
    onnx_path = output_dir / "synthesizer_model.onnx"
    ...
    manifest = {
        "enabled": False,
        "version": "0.1.0",
        "modelFile": "synthesizer_model.onnx",
...
def main():
    parser = argparse.ArgumentParser(description="Export synthesizer model to ONNX")
    parser.add_argument(
        "--model-path",
        type=Path,
        required=True,
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("../../models/agents/synthesizer"),
