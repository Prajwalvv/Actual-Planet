...
class SynthesizerTraceLoader:
    """Loads synthesizer agent traces from JSONL files."""

    def __init__(self, trace_dir: Path):
        self.trace_dir = Path(trace_dir)

    def load_traces(self, agent_type: str = "synthesizer") -> List[Dict[str, Any]]:
...
class SynthesizerPointwiseDataset(Dataset):
    """
    Pointwise dataset for synthesizer agent.
...
class SynthesizerPairwiseDataset(Dataset):
    """
    Pairwise ranking dataset for synthesizer agent.
    ...
    def _build_samples(self, traces: List[Dict[str, Any]]):
        ...
        for trace in traces:
            ...
            if chosen_features is None or len(chosen_features) != 40:
                continue
            ...
            for i, candidate in enumerate(candidates):
                ...
                if neg_features is None or len(neg_features) != 40:
                    continue
...
def compute_feature_stats(traces: List[Dict[str, Any]]) -> Dict[str, np.ndarray]:
    ...
    Returns dict with 'mean' and 'std' arrays of shape [40].
    """
    all_features = []
    ...
    if not all_features:
        ...
        return {"mean": np.zeros(40), "std": np.ones(40)}
...
if __name__ == "__main__":
    ...
    if traces:
        ...
        pointwise_ds = SynthesizerPointwiseDataset(train_traces)
        ...
        pairwise_ds = SynthesizerPairwiseDataset(train_traces)
