"""
SYNTHESIZER AGENT TRAINING SCRIPT

Trains the synthesizer agent model using imitation learning from agent traces.
Supports both pairwise ranking loss and pointwise BCE loss.

Usage:
    python train_synthesizer.py --trace-dir ../../traces/agents --epochs 50
"""
...
from synthesizer_model import SynthesizerPointwiseModel, SynthesizerPairwiseWrapper, count_parameters
from synthesizer_dataset import (
    SynthesizerTraceLoader,
    SynthesizerPointwiseDataset,
    SynthesizerPairwiseDataset,
...
def train_pairwise_epoch(
    model: SynthesizerPairwiseWrapper,
...
def train_pointwise_epoch(
    model: SynthesizerPointwiseModel,
    dataloader: DataLoader,
    optimizer: optim.Optimizer,
    device: torch.device,
) -> float:
...
def train_synthesizer(
    ...
    # Load traces
    loader = SynthesizerTraceLoader(trace_dir)
    traces = loader.load_traces("synthesizer")
    ...
    # Create datasets
    if mode == "pairwise":
        train_dataset = SynthesizerPairwiseDataset(train_traces)
        val_dataset = SynthesizerPairwiseDataset(val_traces)
    else:
        train_dataset = SynthesizerPointwiseDataset(train_traces)
        val_dataset = SynthesizerPointwiseDataset(val_traces)
    
    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True)
    val_loader = DataLoader(val_dataset, batch_size=batch_size, shuffle=False)
    
    # Create model
    pointwise_model = SynthesizerPointwiseModel(input_dim=40, hidden_dim=64)
    print(f"Model parameters: {count_parameters(pointwise_model):,}")
    
    if mode == "pairwise":
        model = SynthesizerPairwiseWrapper(pointwise_model)
    ...
    model_path = output_dir / "synthesizer_model.pt"
...
def main():
    parser = argparse.ArgumentParser(description="Train synthesizer agent model")
    parser.add_argument("--trace-dir", type=Path, required=True, help="Directory containing trace JSONL files")
    parser.add_argument("--output-dir", type=Path, default=Path("../../models/agents/synthesizer"), help="Output directory")
